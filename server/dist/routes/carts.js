...
router.post("/items", requireAuth, async (req, res) => {
    ...
    try {
        const supabase = getSupabaseAdmin();
        let { data: cart } = await supabase.from("carts").select("*").eq("user_id", req.user.id).maybeSingle();
        if (!cart) {
            const insert = await supabase.from("carts").insert({ user_id: req.user.id, store_id: parsed.data.store_id }).select("*").single();
            cart = insert.data;
        }
...
router.post("/checkout", requireAuth, async (req, res) => {
    ...
    try {
        ...
        const { data: order, error } = await supabase.from("orders").insert({ customer_id: req.user.id, store_id: cart.store_id, origin: parsed.data.origin, destination: parsed.data.destination, amount, status: "pending", payment_method: parsed.data.payment_method, distance_km: fare.distanceKm }).select("*").single();
        if (error)
            return res.status(500).json({ error: error.message });
        if (items?.length)
