...
router.post("/login", async (req, res) => {
	...
	try {
		...
		if (user.type === "driver" && user.approval_status !== "approved") return res.status(403).json({ error: "Driver not approved" });
...
router.post("/otp/verify", async (req, res) => {
	...
	try {
		...
		if (error || !row) return res.status(400).json({ error: "Invalid code" });
		if (row.consumed_at) return res.status(400).json({ error: "Code already used" });
